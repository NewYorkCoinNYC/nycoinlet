...
newyorkcoin.auth(app.get('newyorkcoin_username'),app.get('newyorkcoin_password')).set('host', 'localhost').set({port:22555})

app.set 'name', 'nycoinlet'
app.set 'admin_email', "support@newyorkcoin.net"
app.set 'email_name', 'NYCoinLet'
...
app.get '/', (req, res)->
  ...
  if email and req.session.address # If we're logged in properly...
    ...
    newyorkcoin.getBalance email, (err, result)->
...
app.get '/signup', (req,res)->
  ...
  newyorkcoin.getAddressesByAccount email, (err, result)->
    ...
    if result.length
      ...
      mailer.send_mail
          ...
          body:"""
          Here's your secure NYCoinLet link:
      ...
      newyorkcoin.getNewAddress email, (err, result)->
...
app.get '/login', (req,res)->
  ...
  if get_email_hash(req.query.email) is req.query.hash # hash is good.
    ...
    newyorkcoin.getAddressesByAccount req.query.email, (err, result)->
...
app.get '/send', login_required, (req,res)->
  ...
  newyorkcoin.validateAddress req.query.send_to, (err, result)->
    if err
      return security_error '' + err
    if result.isvalid
      newyorkcoin.sendFrom email, req.query.send_to, parseFloat(req.query.amount), 1, ->
        res.send arguments
...
app.get '/history', login_required, (req,res)->
  ...
  newyorkcoin.listTransactions req.session.email, 11, page * 0, (err, transactions)->
    if err
      return security_error '' + err
    res.render 'history',
...
app.get '/history/:tid', login_required, (req,res)->
  ...
  newyorkcoin.gettransaction req.params.tid, (err, transaction)->
    if err
      return security_error '' + err
